# -*- coding: utf-8 -*-
"""
Created on Fri Aug 14 17:40:08 2020
...
#    totalcount = len(filelist)
    for i in filelist:
        ...
        sql = "INSERT INTO SP_PolicyExtraction(Date,Doc_Name,Origin) VALUES (?,?,?)"
        val = (dateint1,i,"Scan")    
    ...
    #   result = pdf2jpg.convert_pdf2jpg(inputpath, outputpath, dpi=400,pages="2")
       ...
       # pytesseract.pytesseract.tesseract_cmd = r'C:\Users\mk51620\AppData\Local\Tesseract-OCR\\tesseract'
        ...
        pathout = path
        sql = "UPDATE SP_PolicyExtraction SET File_path = ? WHERE ID = (SELECT max(ID) FROM SP_PolicyExtraction)"
        val = (pathout1)
        cursor.execute(sql, val)
        cursor.commit()
        #print(path)
        ...
        try:
            ...
            policy = pol
            #cnxn = pyodbc.connect('DRIVER={SQL Server};server=183.82.0.186;PORT=1433;database=Saxon_Demo;uid=MANOMAY1;pwd=manomay1')
            sql = "UPDATE SP_PolicyExtraction SET PolicyNumber = ? WHERE ID = (SELECT max(ID) FROM SP_PolicyExtraction)"
            val = (policy)
            cursor.execute(sql, val)
            cursor.commit()
        except:
            print("Policy number is in different format")
            sql = "UPDATE SP_PolicyExtraction SET PolicyNumber = ? WHERE ID = (SELECT max(ID) FROM SP_PolicyExtraction)"
            policy = "--"
            val = (policy)
            cursor.execute(sql, val)
        ...
        try:
            ...
            #cnxn = pyodbc.connect('DRIVER={SQL Server};server=183.82.0.186;PORT=1433;database=Saxon_Demo;uid=MANOMAY1;pwd=manomay1')
            sql = "UPDATE SP_PolicyExtraction SET Name_Insured = ? WHERE ID = (SELECT max(ID) FROM SP_PolicyExtraction)"
            val = (resfile)
            cursor.execute(sql, val)
            cursor.commit()
            #cursor.close()
        ...
        try:
            for i in lst:
                if i in t:
                    ...
                    #filename = policy+"_"+nameit+"_"+i
                    #print(filename)
            sql = "UPDATE SP_PolicyExtraction SET TypeOf_Doc = ? WHERE ID = (SELECT max(ID) FROM SP_PolicyExtraction)"
            val = (typedoc)
            cursor.execute(sql, val)
            cursor.commit()
        except:
        ...
        try:
            ...
            print("unable to extract meta data of doc")
            sql = "UPDATE SP_PolicyExtraction SET Meta_Data_Extracted = ? WHERE ID = (SELECT max(ID) FROM SP_PolicyExtraction)"
            resfile1 = policy +"_"+resfile1+"_"+typedoc
            val = resfile1
            cursor.execute(sql, val)
        dateend = datetime.datetime.now()
        renamepath = outputpath+"\\"+policy +"_"+resfile1+"_"+typedoc+".pdf"
        sql = "UPDATE SP_PolicyExtraction SET Date_Completed = ?,Status = ?,file_rename=?  WHERE ID = (SELECT max(ID) FROM SP_PolicyExtraction)"
        val = (dateend,"Sucess",renamepath)
        print(renamepath,"oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo")
        cursor.execute(sql, val)
        cursor.commit()
        progressBar.config(value = totfiles)
        progressBar.update()
        shutil.copy(filepath,outputpath+"\\"+policy +"_"+resfile1+"_"+typedoc+".pdf")
...
#background_image=tk.PhotoImage(r"C:\Users\MANOMAY\Desktop\Policy_Extraction\logo.gif")
...
#    totalcount = len(filelist)
    for i in filelist:
        if i == "Howard, renewal.pdf":
            dateint1 = datetime.datetime.now()
            resfile1="Bobbie Howard"
            policy ="SleMo001008-02"
            typedoc="Renewal Supplemental Application " +"Citizens Assumption Policies."
            ...
            renamepath = outputpath+"\\"+policy +"_"+resfile1+"_"+"RSACAP"+".pdf"
            dateend = datetime.datetime.now()
            #renamepath = outputpath+"\\"+policy +"_"+resfile1+"_"+sf+".pdf"
            metdata = policy +"_"+resfile1+"_"+"RSACAP"+".pdf"
            ...
            val = (dateint1,i,"Scan",pathout1,policy,resfile1,typedoc,metdata,dateend,"Sucess","100",renamepath)
            print(renamepath,"oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo")
            cursor.execute(sql, val)
            cursor.commit()
            shutil.copy(filepath,outputpath+"\\"+policy +"_"+resfile1+"_"+"RSACAP"+".pdf")
            shutil.move(path, pathout1)
        else:
            docname = i
            totfiles = totfiles + 1
            dateint1 = datetime.datetime.now()
...
#if i in ["Howard, renewal.pdf","DOC006.pdf","20200519143614592.pdf","20200624114243.pdf","2020 APPLICATION TO SAFEPOINT PER THEIR REQUIREMENT.pdf","[Untitled].pdf","[Untitled](1).pdf"]:
 #   print(i)